...
function insertProjectSub( target, information ) {
	...
	information.sub.forEach(( subItem ) => {
		...
		insertSubTitle( textWrapper, subItem.subtitle );
		subItem.descriptions.forEach(( descriptionItem ) => {
			insertSubDescription( textWrapper, descriptionItem );
		});
		...
		let projectSub = subItem.subtitle.replace( /\s+/g, '-' ).toLowerCase();
